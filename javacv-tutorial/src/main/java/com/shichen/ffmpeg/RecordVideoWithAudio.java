 ...
 */
@Slf4j
public class RecordVideoWithAudio {
    /**
     ...
     * @date 2022/08/03
     */
    public static void recordWebcamAndMicrophone(int webcamDeviceIndex, final int audioDeviceIndex, String outputPath,
                                                 int captureWidth, int captureHeight, final int frameRate) throws Exception {
        long startTime = 0;
        long videoTS = 0;
        /**
         ...
         * FFmpegFrameGrabber.
         */
        final OpenCVFrameGrabber grabber = new OpenCVFrameGrabber(webcamDeviceIndex);
         ...
         */
        final FFmpegFrameRecorder recorder = new FFmpegFrameRecorder(outputPath, captureWidth, captureHeight, 2);
        ...
        while (((capturedFrame = grabber.grab()) != null) && (cFrame.isShowing())) {
            ...
            if (videoTS > recorder.getTimestamp()) {
                log.info("Lip-flap correction: " + videoTS + " : " + recorder.getTimestamp() + " -> "
    ...
    public static void main(String[] args) {
        try {
            recordWebcamAndMicrophone(0, 4, "G:\\test\\voice_ai\\test.flv", 1080, 720, 25);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
